...
def main(numt_file, sam_file, min_reads, max_length):
    ...
    numt_df = pd.read_csv(numt_file, names=['CHR', 'START', 'END', 'MSTART', 'MEND', 'MSTRAND', 'LEN'], sep = "\t", engine = 'python')
    ...
    final_df = pd.DataFrame(columns=['CHR', 'START', 'END', 'MSTART', 'MEND', 'MSTRAND', 'LEN', 'Cluster_Start', 'Cluster_End'])
...
if __name__ == '__main__':
    ...
    # VCF file argument
    parser.add_argument('-n', '--numt', type=str, required=True, help='Input table containing NUMTs Genome Coordinates.')

    # SAM file argument
    parser.add_argument('-s', '--sam', type=str, required=True, help='Input Discordant SAM file.')
    ...
    parser.add_argument('-l', '--max_length', type=str, required=True, help='Maximum Distance to Group NUMTs. Default Value is 500.)')
